...
function makeFour() {
	...
	.then(response=>response.json())
	.then(data=>{
		const activityDescription = data.activity;
...
function makeFive() {
	...
	.then(data=>{
		...
		console.log(`количество участников для активности:${participants}`);
	})
	.catch(error=>console.log(error));
}
...
function makeTen() {
	...
	.then(data=>{
		...
		document.getElementById('result10').appendChild(img);
	})
	.catch(error=>console.log(error));
}
...
function makeEleven() {
	fetch('https://api.ipify.org?format=json')
		.then((response) => response.json())
		.then((data) => {
			document.getElementById('result11').textContent=data.ip;
		})
		.catch((error) => {
			console.error('Ошибка:', error);
		});
...
//Задание 25
//В каком порядке числа выведутся в консоль? -1,3,2
